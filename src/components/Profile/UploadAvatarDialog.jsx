...
const UploadAvatarDialog = (props) => {
    ...
    const { onClose, open, onSelectedAvatar } = props;
    
    const handleSubmit = (e) => {
        e.preventDefault();
        console.log(selectedFile);
        if (selectedFile) {
            const storeageRef = ref(storeage, `files/${currentUser.userId}/avatar/${selectedFile.name}`);
            ...
            uploadTask.on('state_changed', 
                ...
                () => {
                    getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
                        UserService.updateAvatar(currentUser.userId, downloadURL)
                            .then((res) => {
                                onSelectedAvatar(downloadURL);
